...
def train(i, critrion, STEPS, device="cpu"):
    ...
    model = de_model()
    model.to(device)
    model_path = f"./data/ft_stacking_{data_type}_{i}.de.model"
    loss_path = f"./data/ft_stacking_{data_type}_{i}.de.loss"

    X, Y, dataloader, dim_rope_seq, xt, yt = loadStackingData(datatype="d4", shuffle=True, resampleIndex=57, startIndex=0, max_len=60)
...
if __name__ == "__main__":
    ...
    for i in range(N,N+1):
        ...
        plt.savefig(f"./stacking_line_{N+1}.id.png")