...
def train(i, critrion, STEPS, device="cpu"):
    ...
    model = de_model()
    model.to(device)
    model_path = f"./data/ft_stacking_{data_type}_{i}.de.ofd.model"
    loss_path = f"./data/ft_stacking_{data_type}_{i}.de.ofd.loss"
...
if __name__ == "__main__":
    ...
    for i in range(N,N+1):
        ...
        #     lower[0] = y_pred[0] - (np.random.random()*0.5 + 1)
        # if upper[0] - y_pred[0] < limit:
        #     upper[0] = y_pred[0] + (np.random.random()*0.5 + 1)
        # if y_pred[1] - lower[1] < limit:
        #     lower[1] = y_pred[1] - (np.random.random()*0.5 + 1)
        # if upper[1] - y_pred[1] < (np.random.random()*0.5 + 1):
        #     upper[1] = y_pred[1] + (np.random.random()*0.5 + 1)

        # if y_pred[2] - lower[2] < limit:
        #     lower[2] = y_pred[2] - (np.random.random()*0.5 + 1)
        # if upper[2] - y_pred[2] < limit:
        #     upper[2] = y_pred[2] + (np.random.random()*0.5 + 1)
        # if y_pred[3] - lower[3] < limit:
        #     lower[3] = y_pred[3] - (np.random.random()*0.5 + 1)
        # if upper[3] - y_pred[3] < (np.random.random()*0.5 + 1):
        #     upper[3] = y_pred[3] + (np.random.random()*0.5 + 1)
        ...
        plt.savefig(f"./stacking_line_{N+1}.ofd.png")